...
export default function ProspectImport({ repository, existingKeys, onDone, onClose }: Props) {
  ...
  const read = async (file?: File) => {
    ...
    try {
      ...
      parsed.rows = parsed.rows.map(row => ({ ...row, source_metadata: { ...row.source_metadata, 'Aktarım kaynağı': row.source_metadata['Aktarım kaynağı'] || parsed.source_name || 'Google E-Tablo' } }));
  ...
  return <section aria-label="Kayıt aktarımı" className="p-5 rounded-xl border border-sky-200 dark:border-sky-800 bg-white dark:bg-slate-800 space-y-4">
    ...
    {batch && <>
      ...
      <p className="text-sm text-slate-500">Kayıtlar “Liste aktarımı” etiketiyle Portföy Takibi’ne eklenir. Eski notlar ayrı korunur; aktarım, CRM’de görüşme yapılmış sayılmaz. Mevcut Müşteriler listen değişmez.</p>
