...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // ⚡ OPTİMİZE: State'teki planLimits kullanılıyor, DB sorgusu yok
  const canAddProperty = async (): Promise<{ allowed: boolean; message?: string }> => {
    ...
    if (propertyLimit === -1) return { allowed: true }; // Sınırsız plan
    if (properties.length >= propertyLimit) {
      return {
        allowed: false,
        message: `Portföy limitinize (${propertyLimit}) ulaştınız. Pro plana yükselterek sınırsız portföy ekleyebilirsiniz.`
      };
    }
    return { allowed: true };
  };
  ...
  const canAddCustomer = async (): Promise<{ allowed: boolean; message?: string }> => {
    ...
    if (customerLimit === -1) return { allowed: true }; // Sınırsız plan
    if (customers.length >= customerLimit) {
      return {
        allowed: false,
        message: `Müşteri limitinize (${customerLimit}) ulaştınız. Pro plana yükselterek sınırsız müşteri ekleyebilirsiniz.`
      };
    }
    return { allowed: true };
  };
  ...
  const addSale = async (sale: Sale) => {
    ...
    console.log('Saving sale to DB:', saleForDB);
    ...
    // Only add to state after successful DB insert
    if (data) {
      const newSale: Sale = {
        ...sale,
        id: data.id,
  ...
  const deleteSale = async (saleId: string, propertyId?: string) => {
    ...
    // If propertyId provided, also update property status back to Aktif
    if (propertyId) {
      ...
      if (property) {
        ...
        setProperties((prev) => prev.map(p =>
          p.id === propertyId
            ? { ...p, listingStatus: 'Aktif', listing_status: 'Aktif', soldDate: undefined }
            : p
        ));
      }
    }
  ...
  return (
    <DataContext.Provider value={{
      ...
      addSale, deleteSale, updateWebConfig, updateUserProfile,
