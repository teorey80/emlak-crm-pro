...
## 📌 ÖNEMLİ: İçerik/Video Yayın İşleri İçin

Adem bir **YouTube videosu** verip "bunu içeriğe dönüştür / web siteme (ademaslan.com) ekle /
rehber yap" derse, bu CRM dosyası DEĞİL → **`REHBER-ICERIK-YAYIN-AKISI.md`** dosyasını oku ve uygula.
O dosya; Supabase `guides` tablosu, statik üretici, `ademaslan-site-deploy-v5` klasörü ve Netlify
deploy akışını uçtan uca anlatır. (Web sitesi, bu CRM uygulamasından ayrı bir statik projedir.)

> Not: Klasördeki `ademaslan-website/` ve `website-v18/` **eski/ölü** sürümlerdir; sadece
> `ademaslan-site-deploy-v5/` canlıdır.
